	...
	try{
		...
		const decrypted_res =
			!dataType || dataType == 'auto'
				? res.json() || res.blob() || res.text() || function(){return throw new Error('Failed to detect data type')}
				:(
					dataType == 'json'
						? await res.json()
						: (
							dataType == 'blob'
								...
								: (
									dataType == 'text'
										? await res.text()
										: function(){return throw new Error(`Data type ${dataType} couldn't be decrypted`)}
								)
						)
				)
